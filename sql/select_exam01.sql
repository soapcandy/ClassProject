...
select ename, sal, sal + 300 as AD_SAL from emp;
...
select ename, deptno from emp where deptno between 20 and 30 order by ename asc;

-- 8. 사원의 급여가 2000에서 3000사이에 포함되고 부서번호가 20 또는 30인 사원의 이름, 급여와 부서번호를 출력, 이름순(오름차순)으로 출력하시오.
select ename, sal, deptno from emp where (sal between 2000 and 3000)and(deptno between 20 and 30) order by ename asc;
...
select ename, sal, comm from emp where not(comm=0 or comm is null) order by sal desc, comm desc;
